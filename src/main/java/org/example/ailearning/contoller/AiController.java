...
//        })).collect(Collectors.toList());
     ...
     * 多轮对话测试接口
     * @param question 用户问题
     * @param sessionId 会话ID，用于区分不同的对话上下文
     * @return AI 的回答（包含历史对话上下文）
     */
    @GetMapping("/chat-history")
    @ResponseBody
    public String chatWithHistory(@RequestParam String question,
                                   @RequestParam(defaultValue = "default") String sessionId) {
        return aiService.chatWithHistory(sessionId, question);
    }

    /**
     * Function Calling 测试接口（多函数注册）
     * @param question 用户问题
     * @return AI 的回答（LLM 自动选择合适的函数调用）
     */
